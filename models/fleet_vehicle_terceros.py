...
class ResPartner(models.Model):
    ...
    state = fields.Selection(
      selection=[('activo', 'Activo'),('Inactivo', 'Inactivo'),('otro', 'Otro')],
      string="Estado"
    )
    licencia_id = fields.One2many(
        comodel_name='licencia.res.partner',
        inverse_name='partner_id',
        string='licencias'
    )
    documentos_ids = fields.Many2many(
        comodel_name='ir.attachment',
        reverse_name='res_partner_document_rel',
        column1='partner_id',
    ...
    licencia_cancelada = fields.Boolean(
        string="Licencia cancelada",
        default=False
    )
    fecha_cancelacion = fields.Date(
        string="fecha de cancelacion",
        help="Fecha cancelacion de licencia"
    )
    motivo = fields.Text(
        string="Motivo de cancelación",
        help="digite el motivo por el cual la licencia fue cancelada"
    )
...
class TipoLicenciaResPartner(models.Model):
    ...
    servicio = fields.Selection(
        selection=[('particular', 'Servicio particular'),('publico', 'Servicio publico')],
...
class LicenciaResPartner(models.Model):
    ...
    name = fields.Text(
        compute='_compute_licencia_name',
        store=True
    )
    ...
    licencia_id = fields.Many2one(
        comodel_name='tipo.licencia.res.partner',
        string='Tipo de licencia',
        ondelete='restrict'
    )
    restriccion = fields.Char(
        string='Restricción licencia'
    )
