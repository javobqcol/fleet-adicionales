...
class FleetVehicleCost(models.Model):
    ...
    odometer = fields.Float(
        ...
        inverse='_set_odometer1',
        string='Odometro anterior',
        help='Odometro anterior'
    )
    odometer_final = fields.Float(
        compute='_get_odometer',
        inverse='_set_odometer2',
        string='Odometro nuevo',
        help='Odometro nuevo'
    )
    ...
    def _set_odometer1(self):
        for record in self:
              ...
              temp = self.env['fleet.vehicle.odometer'].browse(
                  record.odometer_id.id
              )
        ...
        for record in self:
            if record.amount < 0.0:
                raise ValidationError(
                    "Error, No se pueden asignar costos negativos"
                )
            if not record.date:
                raise ValidationError(
